...
#' @param eangle  early angle. For evenings this is the smaller angle. Mornings is vice/versa
#' @param langle late angle. For evenings this is the larger angle.  Mornings is vice/versa
...
#########################################startwork###########################################
exodus_copy<-function(birdnight, rawfolder, output, radar,langle,eangle,morning=F,years=unique(nights2$year) +1900,folders=T, nexrad_n="V:/Documents/nexrad_site_list_with_utm.csv"){
  #############################################################################################
  require(RAtmosphere)
  #############################################################################################
  ...
  #print(year_time)
  year_angle<-lapply(year_time,function(x){SZA_pn(x,lat,long)})
  badnight<-numeric()
  ...
  for(i in 1:length(nights2)){
    ...
    if(folders==F){file.copy(paste0(rawfolder,'/',year,'/',files[dnight[i]==filed & (fileangle>eangle & fileangle <langle)]),paste0(output))}
    
   if(folders==T){ dir.create(paste0(output,'/',fold),showWarnings=FALSE)
    file.copy(paste0(rawfolder,'/',year,'/',files[dnight[i]==filed & (fileangle>eangle & fileangle <langle)]),paste0(output,'/',substr(nights2[i],1,10)))
  }
